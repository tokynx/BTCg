...
class Address:
    ...
    class MainNet(Net):
        ...
        def generate_publicaddress1_compressed(self):
            self.pubaddr1c = self.instance._generate_publicaddress1_compressed(Address.MainNet.PREFIX_A)
    ...
    class TestNet(Net):
        ...
        def generate_publicaddress1(self):
            self.pubaddr1 = self.instance._generate_publicaddress1(Address.TestNet.PREFIX_A,
                                                                   Address.TestNet.PREFIX_B)

        def generate_publicaddress1_compressed(self):
            self.pubaddr1c = self.instance._generate_publicaddress1_compressed(Address.TestNet.PREFIX_A)
    ...
    def _generate_publicaddress3(self, prefix):
        ...
        p = self._generate_compressed_pubkey()
        redeem_script = hash160(prefix_redeem + hash160(p).digest()).digest()  # 20 bytes
    ...
    def __str__(self, testnet=False):
        ...
        webbrowser.open('https://bitinfocharts.com/bitcoin/address/'+self.mainnet.pubaddrbc1_P2WPKH, new=2)
